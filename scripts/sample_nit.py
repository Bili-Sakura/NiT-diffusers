...
from diffusers import NiTPipeline


def parse_args():
    parser = argparse.ArgumentParser(description="Sample images with a converted Diffusers-style NiT pipeline.")
    parser.add_argument("--model", required=True, help="Path or Hub id of a converted NiT pipeline.")
...
def main():
    ...
    pipe = NiTPipeline.from_pretrained(args.model, torch_dtype=dtype).to(args.device)
    output = pipe(
